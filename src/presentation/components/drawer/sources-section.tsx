        ...
        <div className="space-y-1">
          {refs.map((ref) => (
            ...
            >
              ...
              <CaretRightIcon weight="bold" className="h-3 w-3 shrink-0 text-muted-foreground/30 mt-1 opacity-0 group-hover:opacity-100 transition-fast" />
            </button>
          ))}
        </div>
      </div>
          ...
          {LECTURES.map((lecture) => (
            ...
            >
              ...
              <div className="shrink-0 flex h-9 w-14 items-center justify-center rounded bg-muted/60">
                <YoutubeLogoIcon weight="duotone" className="h-3.5 w-3.5 text-muted-foreground/40" />
              </div>
              <div className="flex-1 min-w-0">
              ...
              <ArrowSquareOutIcon weight="bold" className="h-3 w-3 shrink-0 text-muted-foreground/30 mt-1 opacity-0 group-hover:opacity-100 transition-fast" />
            </button>
          ))}
        </div>
      </div>
...
function SourceLink({ title, source }: { title: string; source: string }) {
  return (
    <button className="group flex w-full items-center gap-2 rounded-lg p-2 text-left transition-fast hover:bg-surface-hover">
      ...
      <ArrowSquareOutIcon weight="bold" className="h-3 w-3 shrink-0 text-muted-foreground/30 opacity-0 group-hover:opacity-100 transition-fast" />
    </button>
  );
}
